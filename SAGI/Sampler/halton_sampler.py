...
class HaltonSampler(object):
    ...
    def __call__(self, trainer, init_code=None, nb_sample=50, labels=None, verbose=True,
                 edit_mask=None, return_final_code=False):
        """
        ...
        Args:
            ...
            edit_mask  -> torch.BoolTensor: If provided, only update masked positions.
            return_final_code -> bool: If True, also return the final token code.

        Returns:
            If return_final_code: (images, l_codes, l_mask, final_code)
            Else: (images, l_codes, l_mask)
        """
        ...
        with torch.no_grad():
            ...
            for index in bar:
                ...
                if self.top_k > 0:# Apply top-k filtering
                    ...
                    # Cast to float32 to avoid bfloat16 simplex validation issues
                    pred_code = torch.distributions.Categorical(probs=prob.float()).sample()

                # Intersect with edit_mask so only originally-masked positions are updated
                if edit_mask is not None:
                    mask = mask & edit_mask.to(mask.device)
        ...
        if return_final_code:
            return x, l_codes, l_mask, code
